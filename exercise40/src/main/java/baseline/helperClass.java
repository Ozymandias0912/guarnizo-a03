...
public class helperClass {
    ...
    public Map<String, Employee> createMap(){
    ...
    public void filter(Map<String, Employee> map , String input){
        ...
        for (Map.Entry m : map.entrySet()) {
            ...
            //compare the input to a substring of the first name and last name of each employee,
            // and print them if they are a match
            if( input.equals( ((Employee)m.getValue()).getFirstName().substring(0, input.length() ))
            || input.equals( ((Employee)m.getValue()).getLastName().substring(0, input.length() ))){
